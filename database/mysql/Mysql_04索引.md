...
3. 业内流传的有一些mysql 军规，其实这些并不是真正的军规，只是典型场景下的最佳实践。真正的军规其实就一条：高效的满足业务需求.
...
- https://www.cnblogs.com/yuyutianxia/p/8284874.html

 MRR的优化在于，并不是每次通过辅助索引就回表去取记录，而是将其rowid给缓存起来，然后对rowid进行排序后，再去访问记录，这样就能将随机I/O转化为顺序I/O，从而大幅地提升性能。
...
9. 10 个升序的数字插入 `2-3树` ,则 树的高度是多少 ?

   - 树高为 2

10. `B-tree` 和 `B+tree` 有什么关联和区别 ?

   - 可以看到B树是2-3树的一种扩展，他允许一个节点有多于2个的元素。
   - `B+ tree` 是`B tree` 的变形树

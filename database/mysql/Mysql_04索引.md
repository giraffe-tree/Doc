...
使用索引可以加快查询速度. 但尽管为查询中使用的每个可能列创建索引很有诱惑力，但不必要的索引会浪费空间并浪费时间让MySQL确定要使用的索引。索引还会增加插入，更新和删除的成本，因为必须更新每个索引。必须找到适当的平衡，以使用最佳索引集实现快速查询。(翻译自mysql8.0的官方文档)  
...
5. 10 个升序的数字插入 `2-3树` ,则 树的高度是多少 ?
   - 树高为 2
6. `B-tree` 是什么 ?
7. `B+树` 是怎么实现的 ?
8. 为什么使用 `B+树`
9. `B-tree` 和 `B+tree` 有什么关联和区别 ?
   - http://www.cnblogs.com/yangecnu/p/Introduce-B-Tree-and-B-Plus-Tree.html
10. 为什么要使用 `N叉树` 
11. “N叉树”的N值在MySQL中是可以被人工调整的么？
12. `lsm 树` 是什么

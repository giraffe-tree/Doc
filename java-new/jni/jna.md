...
但还可以继续调试, 查看变量啥的, 没啥大问题

### dll 兼容性问题

这星期遇到一个在 windows server 2012 上部署的问题, 在本地运行地好好的, 到服务器上就 `UnsatisfiedLinkError` 了.  

这种情况大部分是依赖问题, 找不到某些依赖的 dll

可以通过 `dependency walker` 找到缺失的依赖, 比如我缺失了一个 `vc140.dll` 的vc运行时, 上 https://support.microsoft.com/en-us/help/2977003/the-latest-supported-visual-c-downloads  装一个就可以正常调用了
